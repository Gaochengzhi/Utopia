...
/**
 * onError handler for <img> tags — swaps CDN URL to fallback rewrite path.
 * Usage: <img src={cdnUrl} onError={handleCdnError} />
 *
 * Only retries once (sets data-fallback to prevent infinite loop).
 */
export function handleCdnError(e) {
  const img = e.currentTarget || e.target
  if (!img || img.dataset.fallback) return  // already tried fallback
