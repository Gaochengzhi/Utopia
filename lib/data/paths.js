...
export async function getFolderContents(db, folderPath) {
  const { results: children } = await db.prepare(`
    SELECT pt.*, p.title AS article_title, p.created_at AS article_created_at,
           p.updated_at AS article_updated_at, p.content_preview
    FROM path_tree pt
    LEFT JOIN posts p ON p.slug = pt.path
    WHERE pt.parent_path = ?
    ORDER BY pt.type DESC,
             CASE WHEN pt.type = 'file' THEN COALESCE(p.updated_at, p.created_at) END DESC,
             CASE WHEN pt.type = 'folder' THEN pt.title END ASC,
             CASE WHEN pt.type = 'file' THEN pt.title END DESC
  `).bind(folderPath).all()
  ...
  return children.map(row => {
    ...
    return {
      ...
      time: row.article_updated_at ?? row.article_created_at ?? null,
    }
  })
}
